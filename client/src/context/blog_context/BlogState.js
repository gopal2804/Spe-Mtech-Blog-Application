...
export default function BlogState(props){
    ...
    const getBlogs=async()=>{
        try{
            ...
            const res=await axios.get(`http://192.168.58.2:${constant.port}/api/blogs/view`,config);
            dispatch({
    ...
    const getBlogById=async(blogId)=>{
        try{
            ...
            const res=await axios.get(`http://192.168.58.2:${constant.port}/api/blogs/${blogId}`,config);
            dispatch({
    ...
    const createBlog=async(blogData)=>{
        try{
            ...
            const res=await axios.post(`http://192.168.58.2:${constant.port}/api/blogs/create`,blogData,config);
            dispatch({
    ...
    const updateBlog=async(blogData)=>{
        try{
            ...
            const res=await axios.put(`http://192.168.58.2:${constant.port}/api/blogs/update/${blogData._id}`,blogData,config);
            dispatch({
    ...
    const deleteBlog=async(blogId)=>{
        try{
            ...
            const res=await axios.delete(`http://192.168.58.2:${constant.port}/api/blogs/delete/${blogId}`,config);
            dispatch({
